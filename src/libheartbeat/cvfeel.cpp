...
void hb::CvFeel::load_tmpl(bool & is_ok, cv::Mat & dst, char const path[], char const err_msg[]) const
{
	...
	if (dst.empty()) {
		logger::err(typeid(*this), "load_tmpl::I cannot see ", err_msg);
		is_ok = false;
	}
}
...
hb::CvFeel::CvFeel()
	...
	, is_valid(false)
{
	auto & settings = the_settings::Instance();

	load_tmpl(is_ready,  dinosaur_tmpl, settings.get("resources.dinosaur1", "res/dinosaur_stand.bmp"), "myself" );
	load_tmpl(is_ready, dinosaur2_tmpl, settings.get("resources.dinosaur2", "res/dinosaur_down.bmp" ), "myself2");
	load_tmpl(is_ready,      bird_tmpl, settings.get("resources.bird",      "res/bird.bmp"          ), "birds"  );
	load_tmpl(is_ready,    cactus_tmpl, settings.get("resources.cactus",    "res/cactus.bmp"        ), "cactus" );
	load_tmpl(is_ready,   restart_tmpl, settings.get("resources.restart",   "res/restart.bmp"       ), "restart");
}
